...
CODE-NOT-YET-RECEIVED – המערכת לא קיבלה עדיין את הקוד ממקור ה־GAS.
...
GASURL
```
המשתנה צריך להכיל כתובת (URL) של שירות חיצוני (למשל Google Apps Script)
שתפקידו לשלוף את קוד האימות מהמייל ולהחזיר אותו כטקסט נקי.
הקוד אמור להיות בן 6 ספרות.
...
## 🚀 קריאה לשירות
הפנייה מתבצעת לנתיב הראשי של השרת (לדוגמה https://yourapp.example.com/) בשיטת GET,
עם הפרמטרים הבאים:

פרמטר	חובה	תיאור
num	✅	שם המשתמש (לרוב מספר משתמש)
pas	✅	סיסמת המשתמש
mfaid	✅	מזהה שיטת האימות (למשל מזהה שיטת אימות בדוא"ל)
token	⛔ אופציונלי	טוקן קיים, אם ברצונך לבדוק או לאמת אותו מחדש
...
השירות מקבל את הנתונים (num, pas, mfaid, וייתכן גם token).

אם נשלח טוקן, נבדקת תקפותו מול ימות המשיח.

אם תקף → מוחזר ישירות.

אם לא תקף → נשלחת בקשה לאימות חדש.

אם לא נשלח טוקן כלל, מתבצעת התחברות עם שם משתמש וסיסמה לקבלת טוקן חדש.

נשלח קוד אימות למייל.

השירות שהוגדר ב־GASURL מחלץ את הקוד מהמייל ומחזיר אותו.

הקוד נשלח חזרה לימות לאימות סופי.

במידה והכול תקין — מוחזר הטוקן המאומת.
